...
public class OwnerApi {
    ...
    public ResponseEntity<ApiResponse> getAllOwners() {
        try {
            ...
            // Get users with role "OWNER" (adjust based on your role enum/string)
            List<User> owners = userService
                    .findByRole("OWNER");

            List<UserResponse> ownerResponses = owners.stream()
                    .map(UserResponse::fromUser)
                    .toList();

            logger.info("Successfully retrieved {} owners", ownerResponses.size());
            return ResponseEntity.ok(
                    new ApiResponse(true, "Owners retrieved successfully", ownerResponses)
            );

        } catch (Exception e) {
            logger.error("Error retrieving owners: ", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ApiResponse(false, "Error retrieving owners: " + e.getMessage(), null));
        }
    }
